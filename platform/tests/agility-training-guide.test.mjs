...
At level 40 you will be able to pass every obstacle, and you can then expect to gain around 45,000-50,000 experience per hour when tagging every pillar and using the floor spikes trap during the downtime. This is without the use of [[Karamja gloves]]; wearing [[Karamja gloves 2]] or above will increase the experience rates from passing obstacles and trading in tickets by 10%.
Additionally, the floor spike obstacle can be used, which is very low intensity and can achieve approximately 36,000 experience per hour. Using the "Detached Camera" plugin helps. If the nearby [[Ticket Dispenser|ticket dispensers]] are active, tagging them can provide some extra experience.
...
check(rows.length===10,'The guide parser must emit all ten source-backed base candidates, including the level-40 Brimhaven strategy.');
...
check(get('guide:brimhaven:pillars-floor-spikes-downtime')?.minimum_agility===40&&get('guide:brimhaven:pillars-floor-spikes-downtime')?.observed_xp_per_hour_range?.minimum===45000&&get('guide:brimhaven:pillars-floor-spikes-downtime')?.observed_xp_per_hour_range?.maximum===50000,'The pillar-plus-floor-spike strategy must remain an independent level-40 candidate with its full source range.');
check(get('guide:brimhaven:pillars-floor-spikes-downtime')?.observed_xp_per_hour_level_scope?.minimum===40&&get('guide:brimhaven:pillars-floor-spikes-downtime')?.observed_xp_per_hour_level_scope?.maximum===40&&get('guide:brimhaven:pillars-floor-spikes-downtime')?.equipment_variant_expected_rate_published===false,'The level-40 observation must not be extrapolated or converted into an unpublished glove rate.');
...
check(get('guide:brimhaven:floor-spikes-detached')?.optional_policy_axes?.[0]?.key==='nearby_ticket_dispenser_tagging'&&get('guide:brimhaven:floor-spikes-detached')?.optional_policy_axes?.[0]?.incremental_xp_per_hour_published===false,'Optional nearby dispenser tagging must remain a named, unquantified policy axis.');
