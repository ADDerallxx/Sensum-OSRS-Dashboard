...
const successEvidence=parseBrimhavenFloorSpikeSuccessEvidence({obstaclePage,formulaPage,targetBaseAgility:34});
...
check(successEvidence.length===2&&successEvidence.every(row=>row.state==='blocked'&&row.success_formula_published===true&&row.success_formula_parameters_published===false),'The generic Wiki formula must remain distinct from the unpublished floor-spike parameters.');
...
const parameterizedObstacle={...obstaclePage,content:obstaclePage.content.replace('[[Category:Needs skilling success chart]]','{{Skilling success chart|req1=20|low1=32|high1=192}}')},parameterizedEvidence=parseBrimhavenFloorSpikeSuccessEvidence({obstaclePage:parameterizedObstacle,formulaPage,targetBaseAgility:34});
check(parameterizedEvidence.every(row=>row.state==='candidate'&&row.target_condition_blocker===null&&row.success_probability_at_target===87/256),'Revision-pinned low/high inputs must use the verified Wiki interpolation formula without approximation.');
const failureFreeEvidence=parseBrimhavenFloorSpikeSuccessEvidence({obstaclePage,formulaPage,targetBaseAgility:50});
check(failureFreeEvidence.every(row=>row.failure_probability_at_target===0&&row.target_condition_kind==='explicit_failure_free_threshold'),'The explicit no-failure threshold must establish zero failure without requiring interpolation parameters.');
const mismatchedRequirement=parseBrimhavenFloorSpikeSuccessEvidence({obstaclePage:{...parameterizedObstacle,content:parameterizedObstacle.content.replace('req1=20','req1=21')},formulaPage,targetBaseAgility:34});
check(mismatchedRequirement.every(row=>row.state==='blocked'&&row.target_condition_blocker==='success_interpolation_requirement_does_not_match_obstacle_entry'),'A chart requirement that conflicts with obstacle entry must remain an explicit blocker.');
const conflictingMarker=parseBrimhavenFloorSpikeSuccessEvidence({obstaclePage:{...parameterizedObstacle,content:`${parameterizedObstacle.content}\n[[Category:Needs skilling success chart]]`},formulaPage,targetBaseAgility:34});
check(conflictingMarker.every(row=>row.state==='blocked'&&row.target_condition_blocker==='success_chart_conflicts_with_needs_chart_marker'),'Published parameters and a simultaneous missing-chart marker must fail closed as contradictory evidence.');
