...
const independentCoverage={contract:'sensum.rockslide-pairing-universe-coverage-audit.v1',accountIndependent:true,identityUniverseComplete:true,mechanicalCoverageComplete:false,namedGuideRegression:{complete:true,sourceRevision:core.sourceRevision},contentHash:'synthetic-complete-identity-audit'},closed=auditAgilityRockslideGuideMemberCoverage({members:parsed.records,guideCandidates,pairingUniverseCoverage:independentCoverage});
check(closed.pairedActivityUniverseComplete===true&&closed.internalMemberAuditSatisfied===true&&closed.mechanicalCompletenessProven===false&&closed.absoluteBestGate==='eligible_for_rockslide_mechanical_gate','A complete account-independent pairing identity audit may close open guide language without inventing mechanical completeness.');
const staleIndependent=auditAgilityRockslideGuideMemberCoverage({members:parsed.records,guideCandidates,pairingUniverseCoverage:{...independentCoverage,namedGuideRegression:{complete:true,sourceRevision:'older'}}});
check(staleIndependent.internalMemberAuditSatisfied===false&&staleIndependent.blockers.includes('independent_rockslide_pairing_universe_audit_guide_revision_mismatch'),'An independent pairing audit must match the named-guide revision.');
