...
export const ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_MATERIALIZATION_CONTRACT = 'sensum.activity-reference-collection-member-collection-activity-identity-evidence-postgresql-materialization.v1';
export const ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_INPUT_DOMAIN = 'activity-reference-collection-member-collection-activity-identity-evidence';
export const ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_FACT_KIND = 'raw_activity_reference_member_collection_activity_identity_evidence';
...
const expectedEvidenceChannels=[
  'exact_collection_revision_and_row',
  'all_source_located_collection_row_cells',
  'all_direct_collection_row_wikilinks',
  'exact_linked_source_revision',
  'source_relationship_disposition_and_declarations',
  'exact_cross_source_link_target_intersections'
];
...
function compactStatement(record) {
  const observation=record.collectionActivityIdentityCandidateObservations;
  const collection=record.collectionActivityEvidence.collectionSource;
  const payload={
    blockers:record.blockers,
    canonicalActivityIdentity:null,
    canonicalGameEntityIdentity:null,
    collectionActivityIdentity:null,
    ...
    retainedSourceRelationshipClasses:observation.retainedSourceRelationshipClasses,
    repeatabilityState:record.repeatabilityReview.state,
    rowCellCandidateCount:observation.collectionRowCellCandidates.length,
    sharedMainNamespaceLinkTargetCount:observation.sharedMainNamespaceLinkTargets.length,
    state:record.state
  };
  const envelope={recordKey:record.memberCandidateKey,sourceRevision:String(record.sourceRevision),sourceLocator:{memberCandidateKey:record.memberCandidateKey,sourcePageId:record.sourcePageId,collectionPageId:collection.pageId,collectionRevision:collection.revision},payload};
  return {...envelope,sourceUrl:record.sourceUrl,sourceTimestamp:record.sourceTimestamp,contentHash:hash(envelope)};
}

export function validateActivityReferenceMemberCollectionIdentityMaterializationInput({raw,manifest,audit}) {
  const errors=[];
  ...
  if(manifest?.domain!==ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_INPUT_DOMAIN) add('manifest_domain_mismatch');
  if(manifest?.source?.kind!=='revision_pinned_collection_activity_identity_review_evidence_without_identity_or_relationship_inference') add('manifest_source_channel_mismatch');
  ...
  if(audit?.contract!=='sensum.activity-reference-collection-member-collection-activity-identity-evidence-audit.v1') add('audit_contract_mismatch');
  ...
  if(input.expectedSourceRelationshipDispositionCount!==records.length||input.identityEvidencePacketCount!==records.length||input.exactInputOutputSetAndContextMatch!==true) add('audit_record_coverage_mismatch');
  ...
  const coverage=audit?.collectionActivityEvidenceCoverage||{};
  if((coverage.invalidEvidenceMemberCandidateKeys||[]).length||(coverage.invalidSourceLocatorMemberCandidateKeys||[]).length) add('audit_collection_activity_evidence_invalid');
  const observations=audit?.candidateObservationCoverage||{};
  if(observations.collectionActivityIdentityVerdictCount!==0||observations.linkedSubjectRelationshipVerdictCount!==0||(observations.invalidCandidateVerdictMemberCandidateKeys||[]).length) add('audit_candidate_verdict_gate_weakened');
  ...
  if(audit?.policyCoverage?.policy!=='sensum.activity-reference-collection-member-collection-activity-identity-evidence-policy.v1'||json(audit?.policyCoverage?.evidenceChannels)!==json(expectedEvidenceChannels)||(audit?.policyCoverage?.forbiddenPolicyPaths||[]).length) add('audit_policy_gate_invalid');
  if((audit?.accountStateFindings||[]).length) add('audit_account_state_present');
  if(audit?.identityEvidencePacketCoverageComplete!==true) add('audit_packet_coverage_incomplete');
  if(audit?.collectionActivityIdentityReviewComplete!==false||audit?.linkedSubjectRelationshipReviewComplete!==false||audit?.repeatabilityReviewComplete!==false||audit?.requirementsXpTimingAndMechanicsComplete!==false||audit?.completeActivityUniverse!==false||audit?.absoluteBestGate!=='blocked_incomplete_activity_universe') add('audit_review_or_universe_gate_weakened');
  for(const blocker of ['collection_activity_identity_evidence_requires_semantic_disposition','exact_linked_subject_collection_relationship_review_pending','canonical_game_entity_and_activity_identities_not_established','repeatability_and_member_expansion_not_reviewed','requirements_xp_timing_and_mechanics_not_structured','independent_complete_activity_universe_not_established']) if(!(audit?.blockers||[]).includes(blocker)) add(`audit_required_blocker_missing:${blocker}`);

  const sourcesByIdentity=new Map();
  const seen=[];
  const measured={collectionRevisions:0,linkedRevisions:0,rowCells:0,nonEmptyRowCells:0,rowLinks:0,relationshipSignals:0,matchedDeclarations:0,memberLinkAlignments:0,stableAlignments:0,sharedTargets:0,relationshipClasses:{}};
  ...
  for(const record of records) {
    ...
    if(record?.contract!=='sensum.activity-reference-collection-member-collection-activity-identity-evidence.v1') add(`record_contract_mismatch:${key}`);
    ...
    if(record?.state!=='collection_activity_identity_evidence_ready_for_review') add(`record_state_invalid:${key}`);
    ...
    const packet=record?.collectionActivityEvidence;
    const observation=record?.collectionActivityIdentityCandidateObservations;
    if(packet?.evidenceState!=='revision_pinned_collection_row_and_source_role_evidence_review_only'||observation?.collectionActivityIdentityVerdict!==null||observation?.linkedSubjectRelationshipVerdict!==null||observation?.interpretationState!=='candidate_observations_only_no_identity_or_relationship_inference') add(`record_identity_evidence_gate_invalid:${key}`);
    ...
    const stable=(observation?.stableSourceIdentityAlignments||[]).some(row=>Number(row.sourcePageId)===Number(record.sourcePageId)&&String(row.sourceRevision)===String(record.sourceRevision)&&row.sourceContentHash===record.sourceContentHash);
    if(!stable) add(`record_cross_source_identity_alignment_missing:${key}`);
    const memberAligned=(observation?.memberLinkSourceTitleAlignments||[]).some(row=>Number(row.sourcePageId)===Number(record.sourcePageId)&&String(row.sourceRevision)===String(record.sourceRevision)&&row.resolvedTitle===record.resolvedTitle);
    if(!memberAligned) add(`record_member_link_source_alignment_missing:${key}`);
    if(!(observation?.retainedSourceRelationshipClasses||[]).includes(record?.sourceRelationshipDisposition?.relationshipClass)) add(`record_relationship_class_not_retained:${key}`);
    if(!isSha256(record?.sourceRelationshipDispositionContentHash)||!isSha256(record?.contentHash)||record.contentHash!==hash(without(record,'contentHash'))) add(`record_content_hash_mismatch:${key}`);
    ...
    const collection=packet?.collectionSource;
    ...
    measured.collectionRevisions++;
    measured.linkedRevisions++;
    measured.rowCells+=(packet?.allRowCellEvidence||[]).length;
    measured.nonEmptyRowCells+=(packet?.nonEmptyRowCellEvidence||[]).length;
    measured.rowLinks+=(packet?.directRowWikilinkEvidence||[]).length;
    measured.relationshipSignals+=(record?.sourceRelationshipDispositionSignals||[]).length;
    measured.matchedDeclarations+=(record?.sourceRelationshipDispositionSignals||[]).reduce((sum,row)=>sum+(row.matchedDeclarations||[]).length,0);
    measured.memberLinkAlignments+=(observation?.memberLinkSourceTitleAlignments||[]).length;
    measured.stableAlignments+=(observation?.stableSourceIdentityAlignments||[]).length;
    measured.sharedTargets+=(observation?.sharedMainNamespaceLinkTargets||[]).length;
    const relationshipClass=record?.sourceRelationshipDisposition?.relationshipClass;
    measured.relationshipClasses[relationshipClass]=(measured.relationshipClasses[relationshipClass]||0)+1;
    seen.push(key);
  }
  if(unique(seen).length!==seen.length) add('duplicate_record_keys');
  if(Number(coverage.collectionRevisionPinnedCount)!==measured.collectionRevisions||Number(coverage.linkedSourceRevisionPinnedCount)!==measured.linkedRevisions||Number(coverage.allRowCellEvidenceCount)!==measured.rowCells||Number(coverage.nonEmptyRowCellEvidenceCount)!==measured.nonEmptyRowCells||Number(coverage.directRowWikilinkEvidenceCount)!==measured.rowLinks||Number(coverage.sourceRelationshipDispositionSignalCount)!==measured.relationshipSignals||Number(coverage.matchedSourceDeclarationCount)!==measured.matchedDeclarations||json(coverage.relationshipClassCounts||{})!==json(measured.relationshipClasses)) add('audit_collection_activity_evidence_counts_do_not_reconcile');
  if(Number(observations.memberLinkSourceTitleAlignmentCount)!==measured.memberLinkAlignments||Number(observations.stableSourceIdentityAlignmentCount)!==measured.stableAlignments||Number(observations.sharedMainNamespaceLinkTargetCount)!==measured.sharedTargets) add('audit_candidate_observation_counts_do_not_reconcile');
  if(errors.length) throw new Error(`Activity reference-member collection-identity materialization input rejected: ${unique(errors).join(', ')}`);
  ...
  const model={
    contract:ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_MATERIALIZATION_CONTRACT,domain:ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_INPUT_DOMAIN,
    ...
    gates:{identityEvidencePacketCoverageComplete:true,collectionActivityIdentityReviewComplete:false,linkedSubjectRelationshipReviewComplete:false,repeatabilityReviewComplete:false,memberExpansionReviewComplete:false,mechanicsReviewComplete:false,completeActivityUniverse:false,optimizerEligibleRecords:0,automaticVerification:false,semanticReviewRequired:true}
  };
...
export function buildActivityReferenceMemberCollectionIdentityMaterializationSql(model) {
  assert(model?.contract===ACTIVITY_REFERENCE_MEMBER_COLLECTION_IDENTITY_MATERIALIZATION_CONTRACT,'materialization_contract_mismatch');
  ...
  return [
    ...
    `INSERT INTO data_snapshots(id,label,manifest_hash,complete,validation_summary) VALUES ('${model.snapshotId}',${sqlText(`Accepted collection-activity identity review evidence ${model.snapshotDirectory}`)},${sqlText(model.snapshotContentHash)},false,${sqlJson(validationSummary)}) ON CONFLICT (manifest_hash) DO NOTHING;`,
...
export function verifyActivityReferenceMemberCollectionIdentityReconciliation(model,actual) {
  ...
  assert(actual?.snapshotComplete===false,'collection_identity_evidence_snapshot_must_not_claim_complete_world_knowledge');
  ...
  assert(actual?.metrics?.identityEvidencePacketCoverageComplete===true&&actual?.metrics?.collectionActivityIdentityReviewComplete===false&&actual?.metrics?.linkedSubjectRelationshipReviewComplete===false&&actual?.metrics?.repeatabilityReviewComplete===false&&actual?.metrics?.memberExpansionReviewComplete===false&&actual?.metrics?.mechanicsReviewComplete===false&&actual?.metrics?.completeActivityUniverse===false&&Number(actual?.metrics?.optimizerEligibleRecords)===0&&actual?.metrics?.automaticVerification===false&&actual?.metrics?.semanticReviewRequired===true,'reconciliation_semantic_gate_weakened');
  return true;
}
