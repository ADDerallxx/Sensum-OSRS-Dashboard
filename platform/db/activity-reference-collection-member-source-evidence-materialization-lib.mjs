...
export const ACTIVITY_REFERENCE_MEMBER_SOURCE_MATERIALIZATION_CONTRACT = 'sensum.activity-reference-collection-member-source-evidence-postgresql-materialization.v1';
export const ACTIVITY_REFERENCE_MEMBER_SOURCE_INPUT_DOMAIN = 'activity-reference-collection-member-source-evidence';
export const ACTIVITY_REFERENCE_MEMBER_SOURCE_FACT_KIND = 'raw_activity_reference_collection_member_source_evidence';
...
function compactStatement(record) {
  const payload={
    ...
    collectionCandidateKey:record.collectionContext?.collectionCandidateKey||null,
  ...
  const envelope={recordKey:record.memberCandidateKey,sourceRevision:String(record.sourceRevision),sourceLocator:{memberCandidateKey:record.memberCandidateKey,sourcePageId:record.sourcePageId},payload};
  return {...envelope,sourceUrl:record.sourceUrl,sourceTimestamp:record.sourceTimestamp,contentHash:hash(envelope)};
}

export function validateActivityReferenceMemberSourceMaterializationInput({raw,manifest,audit}) {
  const errors=[];
  ...
  if(manifest?.domain!==ACTIVITY_REFERENCE_MEMBER_SOURCE_INPUT_DOMAIN) add('manifest_domain_mismatch');
  if(manifest?.source?.kind!=='revision_pinned_reference_collection_member_semantic_review_packet') add('manifest_source_channel_mismatch');
  ...
  if(audit?.contract!=='sensum.activity-reference-collection-member-source-evidence-audit.v1') add('audit_contract_mismatch');
  ...
  if(hash(manifest?.source?.audit)!==hash(withoutKeys(audit||{},['generatedAt','policies','inputSnapshot','outputSnapshot','contentHash']))) add('manifest_embedded_audit_mismatch');

  const input=audit?.inputCoverage||{};
  if(input.expectedMemberCandidateCount!==records.length||input.sourceEvidenceRecordCount!==records.length||input.exactCandidateAndContextSetMatch!==true) add('audit_record_coverage_mismatch');
  ...
  const alignment=audit?.sourceAlignment||{};
  if(alignment.distinctExpectedRevisionCount!==records.length||alignment.fetchedRevisionCount!==records.length||alignment.fullyAlignedCount!==records.length||alignment.allIdentityContextsAndFetchedSourcesAligned!==true) add('audit_source_alignment_incomplete');
  for(const key of ['duplicateFetchedRevisions','missingFetchedRevisions','identityContextFailureMemberCandidateKeys','failedMemberCandidateKeys']) if((alignment[key]||[]).length) add(`audit_source_alignment_not_exact:${key}`);
  ...
  for(const blocker of ['one_or_more_member_sources_have_no_supported_activity_infobox','member_semantic_identity_review_pending','member_repeatability_review_pending','member_requirements_variants_xp_timing_and_mechanics_not_structured','independent_complete_activity_universe_not_established']) if(!(audit?.blockers||[]).includes(blocker)) add(`audit_required_blocker_missing:${blocker}`);
  const promotion=audit?.semanticPromotionCoverage||{};
  if(promotion.reviewReadyCount!==records.length||promotion.semanticIdentityReviewedCount!==0||promotion.repeatabilityReviewedCount!==0||promotion.canonicalGameEntityIdentityCount!==0||promotion.canonicalActivityIdentityCount!==0||promotion.optimizerEligibleCount!==0||(promotion.unsupportedPromotionMemberCandidateKeys||[]).length) add('audit_optimizer_or_semantic_gate_weakened');
  if((audit?.accountStateFindings||[]).length) add('audit_account_state_present');
  const structural=audit?.structuralEvidenceCoverage||{};
  if((structural.supportedInfoboxCount||0)+(structural.missingSupportedInfoboxMemberCandidateKeys||[]).length!==records.length||(structural.unbalancedSupportedInfoboxMemberCandidateKeys||[]).length) add('audit_structural_coverage_mismatch');
  ...
  for(const record of records) {
    ...
    if(record?.contract!=='sensum.activity-reference-collection-member-source-evidence.v1') add(`record_contract_mismatch:${key}`);
    ...
    if(!record?.collectionContext?.collectionCandidateKey||!record?.membershipClassification) add(`record_collection_context_missing:${key}`);
    if(!isSha256(record?.sourceMemberCandidateContentHash)||!isSha256(record?.contentHash)||record.contentHash!==hash(without(record,'contentHash'))) add(`record_content_hash_mismatch:${key}`);
  ...
  if(errors.length) throw new Error(`Activity reference-member source materialization input rejected: ${unique(errors).join(', ')}`);

  const statements=records.map(compactStatement);
  const missingInfoboxCount=(structural.missingSupportedInfoboxMemberCandidateKeys||[]).length;
  const model={
    contract:ACTIVITY_REFERENCE_MEMBER_SOURCE_MATERIALIZATION_CONTRACT,domain:ACTIVITY_REFERENCE_MEMBER_SOURCE_INPUT_DOMAIN,
    ...
    gates:{sourceEvidenceCoverageComplete:true,supportedActivityInfoboxCount:structural.supportedInfoboxCount,missingSupportedActivityInfoboxCount:missingInfoboxCount,semanticReviewComplete:false,repeatabilityReviewComplete:false,completeActivityUniverse:false,optimizerEligibleRecords:0,automaticVerification:false,semanticReviewRequired:true}
  };
...
function metrics(model) {
  return {contract:model.contract,sources:model.counts.sources,records:model.counts.records,statements:model.counts.statements,skillCoverage:0,skillKeys:[],recordHashAggregate:model.recordHashAggregate,sourceHashAggregate:model.sourceHashAggregate,statementHashAggregate:model.statementHashAggregate,materializationHash:model.materializationHash,...model.gates};
}

export function buildActivityReferenceMemberSourceMaterializationSql(model) {
  assert(model?.contract===ACTIVITY_REFERENCE_MEMBER_SOURCE_MATERIALIZATION_CONTRACT,'materialization_contract_mismatch');
  ...
  return [
    ...
    `INSERT INTO data_snapshots(id,label,manifest_hash,complete,validation_summary) VALUES ('${model.snapshotId}',${sqlText(`Accepted activity reference-member source evidence ${model.snapshotDirectory}`)},${sqlText(model.snapshotContentHash)},false,${sqlJson(validationSummary)}) ON CONFLICT (manifest_hash) DO NOTHING;`,
...
export function verifyActivityReferenceMemberSourceReconciliation(model,actual) {
  ...
  assert(actual?.snapshotComplete===false,'reference_member_source_snapshot_must_not_claim_complete_world_knowledge');
  ...
  assert(actual?.metrics?.sourceEvidenceCoverageComplete===true&&Number(actual?.metrics?.supportedActivityInfoboxCount)+Number(actual?.metrics?.missingSupportedActivityInfoboxCount)===model.counts.records&&actual?.metrics?.semanticReviewComplete===false&&actual?.metrics?.repeatabilityReviewComplete===false&&actual?.metrics?.completeActivityUniverse===false&&Number(actual?.metrics?.optimizerEligibleRecords)===0&&actual?.metrics?.automaticVerification===false&&actual?.metrics?.semanticReviewRequired===true,'reconciliation_semantic_gate_weakened');
  return true;
}
