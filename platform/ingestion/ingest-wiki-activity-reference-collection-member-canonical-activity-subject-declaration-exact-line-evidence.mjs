...
} from './activity-reference-collection-member-canonical-activity-subject-declaration-exact-line-evidence-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-reference-collection-member-canonical-activity-subject-declaration-exact-line-evidence-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'activity-reference-collection-member-canonical-activity-subject-declaration-discovery';
const outputDomain = 'activity-reference-collection-member-canonical-activity-subject-declaration-exact-line-evidence';
...
async function latestInputSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.subjectDeclarationDiscoveryAttemptCoverageComplete !== true
        || manifest.source?.audit?.canonicalActivitySubjectDeclarationDiscoveryCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('subject_declaration_discovery_input_not_publishable');
  ...
  throw new Error('No valid canonical-activity subject-declaration discovery snapshot exists.');
}
...
const revisionRequests = discoverCanonicalActivitySubjectDeclarationExactRevisionRequests(input.rows, policy);
const fetchedPages = await exactRevisionPages(revisionRequests.map(request => request.sourceRevision));
const built = buildCanonicalActivitySubjectDeclarationExactLineEvidence({ discoveryRecords: input.rows, fetchedPages, policy, contentHash: hash });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'revision_pinned_complete_source_exact_canonical_activity_phrase_occurrence_inventory_without_semantic_or_downstream_promotion',
  api: WIKI_API,
  fetchMode: 'exact retained revision IDs for every canonical-activity subject-declaration discovery candidate, deduplicated by revision',
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    exactLineEvidenceAttemptCoverageComplete: report.exactLineEvidenceAttemptCoverageComplete,
    canonicalActivitySubjectDeclarationExactLineEvidenceCoverageComplete: report.canonicalActivitySubjectDeclarationExactLineEvidenceCoverageComplete,
