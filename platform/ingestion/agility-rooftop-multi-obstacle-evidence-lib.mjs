...
function obstacleEvidence(page,{key,courseName,xpField='xp'}){
  ...
  return {
    ...
    success_probability_at_target:null,
    failure_probability_at_target:null,
...
export function parseAlKharidMultiObstacleFailureEvidence({coursePage,tightropePage,zipLinePage,targetBaseAgility=34}){
  ...
  const target=number(targetBaseAgility),entryLevel=number(entry.match[1]),damage={minimum:number(failure.match[3]),maximum:number(failure.match[4])},tightrope=obstacleEvidence(tightropePage,{key:'tightrope_1',courseName:'Al Kharid Rooftop Course',xpField:'xp1'}),zipLine=obstacleEvidence(zipLinePage,{key:'zip_line',courseName:'Al Kharid Rooftop Course'});
  ...
  const targetConditionBlockers=[];
  if(target<entryLevel)targetConditionBlockers.push('target_below_al_kharid_course_entry_level');
  for(const obstacle of obstacleRows){
    const suffix=obstacle.success_chart_present?'not_verified':'not_published';
    targetConditionBlockers.push(`${slug(obstacle.obstacle_key)}_success_probability_at_base_level_${target}_${suffix}`);
  }
  targetConditionBlockers.push('failed_obstacle_xp_outcome_not_published','failure_recovery_route_and_time_penalty_not_published',...sourceConflicts.map(conflict=>conflict.rule));
  return [{
    contract:'sensum.agility-rooftop-multi-obstacle-failure-evidence.v1',
    ...
    aggregate_failure_probability_at_target:null,
    ...
    target_condition_blockers:[...new Set(targetConditionBlockers)],
