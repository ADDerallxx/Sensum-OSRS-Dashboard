...
import { buildAgilityTargetConditionSourceSufficiency } from './agility-target-condition-source-sufficiency-lib.mjs';
...
const policyFile = path.resolve('platform/policies/agility-target-condition-source-sufficiency-v1.json');
...
const built = buildAgilityTargetConditionSourceSufficiency({ coverageReport, exactRevisionPages, policy, contentHash: hash });
const generatedAt = new Date().toISOString();
let outputSnapshot = null;
if (built.audit.publishable) {
  const snapshot = await writeSnapshot(root, 'agility-target-condition-source-sufficiency', built.records, {
    kind: 'revision_bound_target_condition_source_channel_sufficiency',
    api: WIKI_API,
    fetchMode: 'exact declared revisions only',
    scope: 'five level-34 target-condition gaps; not a complete Wiki or activity universe',
...
const reportDir = path.join(root, 'agility-target-condition-source-sufficiency-audits', generatedAt.replace(/[:.]/g, '-'));
