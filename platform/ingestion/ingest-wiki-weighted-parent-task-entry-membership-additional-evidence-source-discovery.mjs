...
} from './weighted-parent-task-entry-membership-additional-evidence-source-discovery-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/weighted-parent-task-entry-membership-additional-evidence-source-discovery-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'weighted-parent-task-entry-membership-additional-evidence-work-queue';
const outputDomain = 'weighted-parent-task-entry-membership-additional-evidence-source-discovery';
...
async function latestInputSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (audit.queueExportComplete !== true || audit.weightedMembershipReviewComplete !== false || audit.optimizerEligibleCount !== 0 || audit.completeActivityUniverse !== false || audit.publishable !== true) reasons.push('additional_evidence_queue_not_publishable_or_gates_not_closed');
  ...
  throw new Error('No valid weighted membership additional-evidence work queue snapshot exists.');
}
...
const revisionIds = [...new Set(input.rows.flatMap(record => (record.evidenceSourceIdentities || []).map(identity => identity.sourceRevision)).filter(Boolean).map(String))].sort();
const pinnedRevisionPages = await exactRevisionPages(revisionIds);
const discoveryResponses = [];
const candidateRequests = [];
for (const record of input.rows) {
  const queryDefinitions = buildWeightedMembershipAdditionalEvidenceDiscoveryQueries(record, policy, pinnedRevisionPages);
  const searchResponses = [];
  for (const query of queryDefinitions) searchResponses.push(await exactSourceSearch(query));
  discoveryResponses.push({ workQueueEntryKey: record.queueEntryKey, searchResponses });
  candidateRequests.push({
    workQueueEntryKey: record.queueEntryKey,
    requests: discoverWeightedMembershipCandidateRequests(record, searchResponses)
  });
}
...
const built = buildWeightedMembershipAdditionalEvidenceSourceDiscovery({
...
const source = {
  kind: 'revision_pinned_official_source_discovery_for_weighted_parent_task_membership_without_review_or_promotion',
  api: WIKI_API,
  fetchMode: 'exact pinned revision revalidation plus fully enumerated source-derived current-revision searches and direct role-applicable source capture',
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    sourceObservationCoverage: report.sourceObservationCoverage,
    ...
    candidateSpecificMembershipEvidenceComplete: report.candidateSpecificMembershipEvidenceComplete,
