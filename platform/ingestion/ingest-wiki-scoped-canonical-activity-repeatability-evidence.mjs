...
} from './scoped-canonical-activity-repeatability-evidence-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/scoped-canonical-activity-repeatability-evidence-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'scoped-canonical-activity-repeatability-evidence-work-routing';
const outputDomain = 'scoped-canonical-activity-repeatability-evidence';
...
async function latestInputSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.routingCoverageComplete !== true
        || manifest.source?.audit?.canonicalActivityScopeReviewComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('repeatability_evidence_work_routing_not_publishable');
  ...
  throw new Error('No valid scoped canonical-activity repeatability-evidence routing snapshot exists.');
}
...
const revisionRequests = discoverScopedCanonicalActivityRepeatabilityExactRevisionRequests(input.rows, policy);
const fetchedPages = await exactRevisionPages(revisionRequests.map(request => request.sourceRevision));
const built = buildScopedCanonicalActivityRepeatabilityEvidence({ routingRecords: input.rows, fetchedPages, policy, contentHash: hash });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'revision_pinned_complete_scoped_activity_repeatability_observations_with_parent_member_and_limit_domains_separate_and_no_verdict',
  api: WIKI_API,
  fetchMode: 'exact canonical activity subject-binding revision IDs with complete source text, line inventory, generic signal candidates, and prior scope-packet alignment',
