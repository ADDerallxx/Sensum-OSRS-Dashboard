...
export function parseBarbarianFishingVariants({pages}){
  ...
  for(const rate of trainingRates){
    ...
    records.push({...base,record_key:`agility-barbarian-fishing:afk:${rate.FishingLevel}`,variant_key:`afk_fishing_${rate.FishingLevel}`,name:`Barbarian Fishing — AFK drop — Fishing ${rate.FishingLevel}`,interaction_policy:'afk_drop',intensity:'low_effort',cycle_ticks:null,xp_per_success:xpPerSuccess,observed_xp_per_hour:rate.afk.Other,observed_xp_per_hour_by_skill:{...rateBySkill(rate.afk),Total:rate.afk.Total}});
