...
const titles=['Yanille Agility Dungeon'];
...
const report=audit(records,{minimum:1,required:['parent_name','variant_key','name','entry_level','xp_per_success','failure_free_level','observed_xp_per_hour','source_revision','source_locator'],maximumUnknownRatio:0});
...
console.log(JSON.stringify({manifest:snapshot.manifest,publishable:report.publishable,records:records.length,variants:records.map(row=>({name:row.name,entryLevel:row.entry_level,failureFreeLevel:row.failure_free_level,observedRateMinimumLevel:row.observed_rate_minimum_level})),findings:report.findings},null,2));
