...
export function parseAgilityTrainingGuideExpansionCandidates({title,content,sourceRevision,sourceTimestamp,sourceUrl}){
  if(title!=='Agility training')return {records:[],audit:{expectedSectionCount:9,parsedSectionCount:0,expectedCandidateCount:10,parsedCandidateCount:0,blockers:['unexpected_source_title'],publishable:false}};
  ...
  if(rockslide)records.push(record(base,{candidate_key:'guide:rockslide-hybrid-detour',source_section_key:sectionKey('Fastest experience',rockslide),name:'Rockslide shortcut — Hybrid activity detour',record_kind:'hybrid_training_method',minimum_agility:number(rockslide.match[1]),requirements:['The Blood Moon Rises'],xp_per_detour:number(rockslide.match[2]),source_interval_minutes_approximate:number(rockslide.match[3]),detour_seconds_range:{minimum:number(rockslide.match[4]),maximum:number(rockslide.match[5])},effective_xp_per_hour_range:{minimum:number(rockslide.match[6]),maximum:number(rockslide.match[7])},incremental_xp_per_hour_range:{minimum:number(rockslide.match[8]),maximum:number(rockslide.match[9])},effective_rate_is_not_sustained_training_rate:true,requires_return_activity_with_nearby_teleport:true,internal_member_audit_required:true,source_locator:locator(rockslide)}));
  ...
  return {records,audit:{expectedSectionCount:9,parsedSectionCount:9-missing.length,expectedCandidateCount:10,parsedCandidateCount:records.length,missingSections:missing,duplicateCandidateKeys,blockers,publishable:blockers.length===0}};
}
