...
} from './agility-target-condition-gap-template-module-source-discovery-lib.mjs';
...
const policyFile = path.resolve('platform/policies/agility-target-condition-gap-template-module-source-discovery-v1.json');
...
const resolvedPolicy = resolveAgilityTargetConditionGapTemplateModuleSourceDiscoveryPolicy(policy, basePolicy);
...
const built = buildAgilityTargetConditionGapTemplateModuleSourceDiscovery({
  coverageReport,
...
if (built.audit.publishable) {
  const snapshot = await writeSnapshot(root, policy.outputDomain, built.records, {
    kind: 'query_bounded_revision_pinned_official_wiki_template_calculator_and_module_source_discovery',
    api: WIKI_API,
    scope: '17 blockers across five level-34 target-condition gaps; Template, Calculator, and Module namespaces only; not a complete Wiki or game-fact universe',
