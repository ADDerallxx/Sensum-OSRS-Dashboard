...
const requiredRules = [
  ...
  'policyCandidateSetMustEqualEveryTargetConditionGap',
...
const excerpt = value => String(value || '').trim().slice(0, 500);

function sourceLineSignals(sourceEntry, predicate, signalKind) {
  const results = [];
  const contentLines = lines(sourceEntry.content);
  for (let index = 0; index < contentLines.length; index += 1) {
    if (!predicate(contentLines[index], sourceEntry)) continue;
    results.push({
      signalKind,
...
function successChartParameterSignals(sourceEntry) {
  const results = [];
  const content = String(sourceEntry.content || '');
  for (const match of content.matchAll(/\{\{Skilling success chart[\s\S]{0,2500}?\}\}/gi)) {
    if (!/\|\s*low\d*\s*=\s*\d+/i.test(match[0]) || !/\|\s*high\d*\s*=\s*\d+/i.test(match[0])) continue;
    results.push({
...
function diagnosticSignals(kind, sourceEntries, targetBaseAgility) {
  const target = String(targetBaseAgility);
  if (kind === 'success_chart_parameters') return sourceEntries.flatMap(successChartParameterSignals);
  if (kind === 'target_numeric_probability') {
    return sourceEntries.flatMap(entry => sourceLineSignals(
      entry,
      line => new RegExp(`(?:\\blevel\\s*${target}\\b|\\b${target}\\s+(?:base\\s+)?Agility\\b)`, 'i').test(line)
        && /(?:chance|probab|success|fail)/i.test(line),
      kind
    ));
  }
  if (kind === 'failed_attempt_xp') {
    return sourceEntries.flatMap(entry => sourceLineSignals(entry, line => /(?:fail(?:ed|ing|ure)?[^\n]{0,140}(?:awards?|grants?|receiv(?:e|es|ed)|\bXP\b|experience(?!\s+per\s+hour))|(?:\bXP\b|experience)[^\n]{0,140}(?:on|upon|for|when|after)\s+(?:a\s+)?fail)/i.test(line), kind));
  }
  if (kind === 'failure_recovery') {
    return sourceEntries.flatMap(entry => sourceLineSignals(entry, line => /fail(?:ed|ing|ure)?[^\n]{0,180}(?:return(?:ed)?|restart|recover|resume|land(?:s|ed)?|fall(?:s|ing)?\s+(?:back|to|into)|walk\s+past|cross[^\n]{0,50}\s+again)/i.test(line), kind));
  }
  if (kind === 'exact_target_rate') {
    return sourceEntries.flatMap(entry => sourceLineSignals(
      entry,
      line => new RegExp(`(?:\\blevel\\s*${target}\\b|\\b${target}\\s+(?:base\\s+)?Agility\\b)`, 'i').test(line)
        && /(?:experience\s+per\s+hour|XP\s*\/\s*(?:h|hr))/i.test(line),
      kind
    ));
  }
  if (kind === 'detached_rate_equipment_scope') {
    return sourceEntries.flatMap(entry => sourceLineSignals(
      entry,
      line => /36,?000/.test(line)
        && /(?:Karamja gloves|diary|equipment|wearing|without|with\s+(?:gloves|diary))/i.test(line),
      kind
    ));
  }
  if (kind === 'shayzien_failure_capable_identity') {
    const allowedTitles = /^(?:Ladder|Monkeybars|Bar|Gap) \(Shayzien Agility Course\)$/;
    return sourceEntries
      .filter(entry => allowedTitles.test(entry.source.title))
      .flatMap(entry => sourceLineSignals(
        entry,
        line => /fail/i.test(line) && !/(?:cannot|never|no longer)\s+(?:be\s+)?fail/i.test(line),
        kind
      ));
  }
  if (kind === 'shayzien_rate_alignment') {
    const course = sourceEntries.find(entry => entry.source.title === 'Shayzien Agility Course');
    const overview = sourceEntries.find(entry => entry.source.title === 'Agility');
    const courseLine = lines(course?.content).findIndex(line => /increased from\s+8,?750\s+to\s+10,?000/i.test(line));
    const overviewLine = lines(overview?.content).findIndex(line => /average Agility experience per hour\s+10,?000/i.test(line));
    if (courseLine < 0 || overviewLine < 0) return [];
    return [{
      signalKind: kind,
      sourceKey: `${course.source.sourceKey}+${overview.source.sourceKey}`,
      title: `${course.source.title} + ${overview.source.title}`,
      sourceRevision: `${course.source.revision}+${overview.source.revision}`,
      line: overviewLine + 1,
      excerpt: excerpt(lines(overview.content)[overviewLine])
    }];
  }
  return [];
}
...
function contextSignals(sourceEntries) {
  const definitions = [
    ['needs_success_chart_marker', line => /\[\[Category:Needs skilling success chart\]\]/i.test(line)],
    ['qualitative_failure_statement', line => /fail/i.test(line)],
    ['rate_statement', line => /(?:experience\s+per\s+hour|XP\s*\/\s*(?:h|hr))/i.test(line)]
  ];
  const found = [];
  for (const [kind, predicate] of definitions) {
    for (const entry of sourceEntries) found.push(...sourceLineSignals(entry, predicate, kind));
  }
...
function policyErrors(policy = {}) {
  ...
  for (const source of policy.sources || []) {
    if (!source.sourceKey || !source.title || !/^[1-9]\d*$/.test(String(source.revision)) || !source.url?.startsWith(policy.acceptedSourceOrigin || 'https://oldschool.runescape.wiki/w/') || !source.channel) errors.push(`invalidSource:${source.sourceKey || 'unknown'}`);
  }
  for (const candidate of policy.candidates || []) {
    ...
    for (const [kind, diagnosticSourceKeys] of Object.entries(candidate.diagnosticSourceKeysByKind || {})) {
      if (!diagnosticSourceKeys?.length) errors.push(`emptyDiagnosticSourceSet:${candidate.candidateKey}:${kind}`);
      for (const sourceKey of diagnosticSourceKeys || []) {
        if (!candidate.sourceKeys.includes(sourceKey)) errors.push(`diagnosticSourceOutsideCandidateSet:${candidate.candidateKey}:${kind}:${sourceKey}`);
      }
    }
  }
  ...
  return (policy.candidates || []).map(candidatePolicy => {
    ...
    const evidenceDomains = (candidate.blockers || []).map(blocker => {
      ...
      const resolutionSignals = diagnosticKinds.flatMap(kind => {
        ...
        return diagnosticSignals(kind, sourceEntries.filter(entry => selectedKeys.includes(entry.source?.sourceKey)), policy.targetBaseAgility);
      });
      return {
        blocker,
        ...
        disposition: resolutionSignals.length
          ? 'potential_condition_evidence_requires_manual_reaudit'
    ...
    const base = {
      ...
      sourceBindings,
      contextSignals: contextSignals(sourceEntries.filter(entry => ['activity_page', 'obstacle_page'].includes(entry.source?.channel))),
      evidenceDomains,
      existingBlockers: sorted(candidate.blockers || []),
      disposition: manualReauditRequired
        ? 'blocked_pending_manual_semantic_reaudit'
        : 'blocked_no_condition_matched_evidence_in_declared_revision_bound_channels',
...
export function buildAgilityTargetConditionSourceSufficiency({ coverageReport = {}, exactRevisionPages = [], policy = {}, contentHash = value => value }) {
  const records = constructRecords({ coverageReport, exactRevisionPages, policy, contentHash });
  return {
    records,
    audit: auditAgilityTargetConditionSourceSufficiency(records, { coverageReport, exactRevisionPages, policy, contentHash })
  };
}

export function auditAgilityTargetConditionSourceSufficiency(records = [], { coverageReport = {}, exactRevisionPages = [], policy = {}, contentHash = value => value } = {}) {
  ...
  const inputTargetCandidates = (coverageReport.details || []).filter(detail => detail.status === 'target_condition_gap');
  const inputKeys = sorted(inputTargetCandidates.map(candidate => candidate.candidateKey));
  ...
  const inputByCandidate = new Map(inputTargetCandidates.map(candidate => [candidate.candidateKey, candidate]));
  ...
  const preservedBlockers = records.every(record => same(
    sorted(record.existingBlockers || []),
    sorted(inputByCandidate.get(record.candidateKey)?.blockers || [])
  ));
  ...
  if (!candidateSetMatches) blockers.push('target_condition_candidate_set_mismatch');
  if (!blockerSetsMatch) blockers.push('target_condition_blocker_set_mismatch');
  ...
  return {
    ...
    candidateCoverage: {
      inputTargetConditionGapCount: inputTargetCandidates.length,
