...
} from './activity-reference-collection-member-independent-repeatability-signal-subject-predicate-evidence-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-reference-collection-member-independent-repeatability-signal-subject-predicate-evidence-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));

async function latestSignalScopeEvidenceWorkRoutingSnapshot() {
  const domain = 'activity-reference-collection-member-independent-repeatability-signal-scope-evidence-work-routing';
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.routingCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('signal_scope_evidence_work_routing_input_not_publishable');
      if (rows.some(record => record.contract !== policy.inputContract)) reasons.push('unexpected_signal_scope_evidence_work_routing_contract');
      if (!rows.length) reasons.push('no_signal_scope_evidence_work_routing_records');
      if (reasons.length) {
  ...
  throw new Error('No valid independent repeatability signal-scope evidence-work routing snapshot exists.');
}
...
const input = await latestSignalScopeEvidenceWorkRoutingSnapshot();
const revisionRequests = discoverIndependentRepeatabilitySignalSubjectPredicateRevisionRequests(input.rows, policy);
...
const source = {
  kind: 'revision_pinned_exact_signal_source_line_predicate_span_and_stable_identity_anchor_observations_without_semantic_binding_or_repeatability_verdict',
  api: WIKI_API,
  fetchMode: 'exact retained source revision IDs for every routed independent repeatability signal, deduplicated by revision',
...
const output = path.join(
  root,
  'activity-reference-collection-member-independent-repeatability-signal-subject-predicate-evidence-audits',
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    sourceLineCoverage: report.sourceLineCoverage,
    stableIdentityObservationCoverage: report.stableIdentityObservationCoverage,
    semanticPromotionCoverage: report.semanticPromotionCoverage,
    subjectPredicateEvidenceAttemptCoverageComplete: report.subjectPredicateEvidenceAttemptCoverageComplete,
    independentSignalSubjectPredicateEvidenceCoverageComplete: report.independentSignalSubjectPredicateEvidenceCoverageComplete,
