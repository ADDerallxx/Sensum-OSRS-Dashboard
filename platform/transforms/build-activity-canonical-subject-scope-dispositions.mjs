...
import { buildActivityCanonicalSubjectScopeDispositions } from './activity-canonical-subject-scope-disposition-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-canonical-subject-scope-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'activity-canonical-subject-scope-evidence';
const outputDomain = 'activity-canonical-subject-scope-disposition';
...
async function latestEvidenceSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.canonicalActivityScopeEvidenceCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('canonical_activity_scope_evidence_not_complete_or_publishable');
      if (!rows.length || rows.some(record => record.contract !== policy.inputContract || record.state !== policy.inputState)) reasons.push('unexpected_or_empty_scope_evidence_contract_or_state');
  ...
  throw new Error('No valid complete canonical-activity subject scope-evidence snapshot exists.');
}

const input = await latestEvidenceSnapshot();
const built = buildActivityCanonicalSubjectScopeDispositions({ evidenceRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_revision_pinned_canonical_activity_scope_semantic_disposition_with_repeatability_members_mechanics_and_optimizer_gates_closed',
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    scopeClassificationCoverage: report.scopeClassificationCoverage,
