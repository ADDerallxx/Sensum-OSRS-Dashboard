...
import { buildWeightedMembershipAdditionalEvidenceSufficiencyDispositions } from './weighted-parent-task-entry-membership-additional-evidence-sufficiency-disposition-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/weighted-parent-task-entry-membership-additional-evidence-sufficiency-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'weighted-parent-task-entry-membership-additional-evidence-source-discovery';
const outputDomain = 'weighted-parent-task-entry-membership-additional-evidence-sufficiency-disposition';
...
async function latestInputSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (audit.evidencePacketAttemptCoverageComplete !== true || audit.weightedMembershipReviewComplete !== false || audit.optimizerEligibleCount !== 0 || audit.completeActivityUniverse !== false || audit.publishable !== true) reasons.push('source_discovery_snapshot_not_publishable_or_gates_not_closed');
  ...
  throw new Error('No valid weighted membership additional-evidence source discovery snapshot exists.');
}

const input = await latestInputSnapshot();
const built = buildWeightedMembershipAdditionalEvidenceSufficiencyDispositions({
...
const source = {
  kind: 'generic_fail_closed_weighted_membership_additional_evidence_sufficiency_disposition_without_review_or_promotion',
