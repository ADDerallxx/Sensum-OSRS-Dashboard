...
const [sections,candidates]=await Promise.all([latestRootSnapshot('agility-training-guide-sections'),latestRootSnapshot('agility-level34-candidates')]),report=auditAgilityTrainingGuideSectionCoverage({sections:sections.rows,candidates:candidates.rows});
report.generatedAt=new Date().toISOString();report.inputSnapshots={sections:{directory:sections.directory,contentHash:sections.manifest.contentHash},candidates:{directory:candidates.directory,contentHash:candidates.manifest.contentHash}};report.snapshotRejections=[...sections.rejections,...candidates.rejections];report.contentHash=hash({...report,contentHash:undefined});
