...
import { buildActivityReferenceCollectionMemberRepeatabilityDispositions } from './activity-reference-collection-member-repeatability-disposition-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-reference-collection-member-repeatability-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));

async function latestRepeatabilityEvidenceSnapshot() {
  const domain = 'activity-reference-collection-member-repeatability-evidence';
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.evidencePacketAttemptCoverageComplete !== true
        ...
        || manifest.source?.audit?.publishable !== true) reasons.push('repeatability_evidence_input_not_publishable');
      if (rows.some(record => record.contract !== policy.inputContract)) reasons.push('unexpected_repeatability_evidence_contract');
      if (!rows.length) reasons.push('no_repeatability_evidence_records');
      if (reasons.length) {
  ...
  throw new Error('No valid collection-activity repeatability-evidence snapshot exists.');
}

const input = await latestRepeatabilityEvidenceSnapshot();
const built = buildActivityReferenceCollectionMemberRepeatabilityDispositions({ repeatabilityEvidenceRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_source_scoped_repeatability_disposition_without_member_mechanics_or_optimizer_promotion',
...
const snapshot = await writeSnapshot(
  root,
  'activity-reference-collection-member-repeatability-disposition',
...
const output = path.join(root, 'activity-reference-collection-member-repeatability-disposition-audits', generatedAt.replace(/[:.]/g, '-'));
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    repeatabilityDispositionCoverage: report.repeatabilityDispositionCoverage,
    semanticPromotionCoverage: report.semanticPromotionCoverage,
    repeatabilityDispositionAttemptCoverageComplete: report.repeatabilityDispositionAttemptCoverageComplete,
