...
import { buildMultiVariantBindingReviewDecisionImport } from './multi-variant-binding-review-decision-import-lib.mjs';
...
const policyFile = 'platform/policies/multi-variant-binding-review-decision-import-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'multi-variant-binding-review-queue';
const outputDomain = 'multi-variant-binding-review-decisions';
...
async function latestQueueSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (audit.queueExportComplete !== true || audit.variantBindingReviewComplete !== false || audit.optimizerEligibleCount !== 0 || audit.completeActivityUniverse !== false || audit.publishable !== true) reasons.push('review_queue_snapshot_not_publishable_or_gates_not_closed');
  ...
  throw new Error('No valid multi-variant binding review queue snapshot exists.');
}
...
const built = buildMultiVariantBindingReviewDecisionImport({
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'explicit_fingerprint_bound_variant_binding_review_decision_recording_without_semantic_promotion',
