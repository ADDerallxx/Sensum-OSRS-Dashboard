...
import { buildUnlockCorpusAbsenceReconciliationHumanReviewPackets } from './cross-skill-rendered-page-without-unlock-evidence-unlock-corpus-absence-reconciliation-human-review-packet-materialization-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/cross-skill-rendered-page-without-unlock-evidence-unlock-corpus-absence-reconciliation-human-review-packet-materialization-v1.json';
...
async function latestQueueSnapshot() {
  ...
  throw new Error('No valid rendered-page/no-unlock level-unlock-corpus absence reconciliation queue snapshot exists.');
}

const input = await latestQueueSnapshot();
const built = buildUnlockCorpusAbsenceReconciliationHumanReviewPackets({
...
const inputSnapshot = {
  ...
  corpusEvidenceContentHash: input.manifest.source?.corpusEvidenceContentHash,
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'deterministic_human_readable_evidence_keyed_unlock_corpus_absence_reconciliation_review_packet_materialization_without_decision_requirement_or_semantic_promotion',
  ...
  const artifacts = {
    directory: 'batches',
    batchCount: built.batchArtifacts.length,
  ...
  console.log(JSON.stringify({
    ...
    coverage: {
      ...
      zeroExactStablePageIdMatchCount: report.evidenceCoverage.zeroExactStablePageIdMatchCount,
      corpusEvidenceBoundCount: report.evidenceCoverage.corpusEvidenceBoundCount,
