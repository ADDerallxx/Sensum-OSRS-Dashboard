...
import { buildUnresolvedRenderedTargetResolutionReviewDecisionImport } from './cross-skill-unresolved-rendered-target-resolution-review-decision-import-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/cross-skill-unresolved-rendered-target-resolution-review-decision-import-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const queueDomain = 'cross-skill-unresolved-rendered-target-resolution-review-queue';
const outputDomain = 'cross-skill-unresolved-rendered-target-resolution-review-decisions';
...
async function latestQueueSnapshot() {
  ...
  for (const directory of await timestampDirectories()) {
    try {
      ...
      if (audit.queueExportComplete !== true || audit.resolutionReviewComplete !== false || audit.completeActivityUniverse !== false || audit.publishable !== true ||
          semantic.recordedReviewCount !== 0 || semantic.selectedResolutionCount !== 0 || semantic.canonicalGameEntityIdentityCount !== 0 ||
      ...
      if (partition.reviewQueueEntryCount !== rows.length || partition.blankDecisionTemplateCount !== rows.length ||
          partition.duplicateQueueDispositionKeys?.length || partition.missingQueueDispositionKeys?.length || partition.unexpectedQueueDispositionKeys?.length ||
          partition.crossQueueDispositionKeys?.length || partition.reviewRecordMismatchKeys?.length || partition.reviewOrderMatches !== true) {
  ...
  throw new Error('No valid unresolved rendered-target resolution review queue snapshot exists.');
}
...
const built = buildUnresolvedRenderedTargetResolutionReviewDecisionImport({
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'explicit_human_source_bound_resolution_review_decision_recording_without_resolution_or_semantic_application',
