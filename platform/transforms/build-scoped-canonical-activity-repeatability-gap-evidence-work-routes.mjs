...
import { buildScopedCanonicalActivityRepeatabilityGapEvidenceWorkRoutes } from './scoped-canonical-activity-repeatability-gap-evidence-work-routing-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/scoped-canonical-activity-repeatability-gap-evidence-work-routing-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'scoped-canonical-activity-repeatability-evidence-disposition';
const outputDomain = 'scoped-canonical-activity-repeatability-gap-evidence-work-routing';
...
async function latestDispositionSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.dispositionAttemptCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('repeatability_disposition_not_complete_or_publishable');
      if (manifest.source?.audit?.repeatabilityClassificationCoverageComplete !== false
        || manifest.source?.audit?.repeatabilityReviewComplete !== false) reasons.push('unexpected_repeatability_classification_or_review_completion');
      if (!rows.length || rows.some(record => record.contract !== policy.inputContract || record.state !== policy.eligibleInputState)) reasons.push('unexpected_or_empty_repeatability_disposition_contract_or_state');
  ...
  throw new Error('No valid unresolved scoped canonical-activity repeatability-disposition snapshot exists.');
}

const input = await latestDispositionSnapshot();
const built = buildScopedCanonicalActivityRepeatabilityGapEvidenceWorkRoutes({ dispositionRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_independent_scoped_activity_repeatability_gap_evidence_work_routing_without_repeatability_or_downstream_promotion',
