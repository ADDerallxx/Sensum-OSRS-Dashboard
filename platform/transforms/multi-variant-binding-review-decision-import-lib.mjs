...
const REQUIRED_RULES = [
  ...
  'submissionMustMatchQueueDispositionPacketEvidenceAndVariantBindings',
  ...
  'recordingADecisionDoesNotBindAVariantOrApplyIdentityMembershipRepeatabilityMechanicsMappingCompletenessOrOptimizerVerdicts',
...
export function compileMultiVariantBindingReviewDecisionImportPolicy(policy = {}) {
  const invalidRules = REQUIRED_RULES.filter(rule => policy.rules?.[rule] !== true);
  if (policy.rules?.automaticVerificationAllowed !== false) invalidRules.push('automaticVerificationAllowed');
  const contractValid = policy.queueContract === 'sensum.multi-variant-binding-review-queue-entry.v1'
    && policy.submissionContract === 'sensum.multi-variant-binding-review-decision-template.v1'
    && policy.recordContract === 'sensum.multi-variant-binding-review-decision.v1'
    && policy.auditContract === 'sensum.multi-variant-binding-review-decision-import-audit.v1'
    && policy.queueState === 'pending_explicit_source_bound_variant_binding_review'
    && policy.recordState === 'recorded_source_bound_variant_binding_review_decision_pending_semantic_disposition';
  const decisions = policy.allowedDecisions || [];
  const decisionsValid = decisions.length === 3
    && decisions[0] === 'confirm_exact_parent_occurrence_to_numbered_variant_binding'
    && decisions[1] === 'reject_exact_parent_occurrence_to_numbered_variant_binding'
...
function queueRecordIntegrity(record = {}, policy = {}, contentHash = hash) {
  ...
  const candidateIndex = record.variantBindingReviewEvidence?.reviewCandidateVariantIndex;
  const checks = {
    ...
    sourceDispositionKeyPresent: typeof record.sourceDispositionKey === 'string' && record.sourceDispositionKey.length > 0,
    evidenceFingerprintPresent: typeof record.evidenceFingerprint === 'string' && record.evidenceFingerprint.length === 64,
    evidencePacketContentHashPresent: typeof record.sourceEvidencePacketContentHash === 'string' && record.sourceEvidencePacketContentHash.length === 64,
    candidateIndexValid: Number.isInteger(candidateIndex) && candidateIndex > 0,
    candidateIndexUnbound: record.boundVariantIndex === null && record.bindingReviewDecision === null,
    semanticGatesClosed: record.candidateMemberIdentityVerdict === null
      && record.parentMembershipVerdict === null
      && record.weightedTaskEntryMembershipVerdict === null
      && record.repeatabilityVerdict === null
      && record.mechanicsReviewComplete === false
    ...
    blankTemplateQueueBindingMatches: template.queueEntryKey === record.queueEntryKey && template.sourceDispositionKey === record.sourceDispositionKey,
    blankTemplateEvidenceBindingMatches: template.sourceEvidencePacketContentHash === record.sourceEvidencePacketContentHash && template.evidenceFingerprint === record.evidenceFingerprint,
    blankTemplateCandidateIndexMatches: template.reviewCandidateVariantIndex === candidateIndex,
    ...
    blankTemplateStateMatches: template.state === 'blank_source_bound_variant_binding_review_decision'
  };
...
function submissionShape(submission = {}) {
  ...
  const allBlank = blank(submission.decision) && blank(submission.reviewer) && blank(submission.reviewedAt) && sourceRevisions.length === 0 && blank(submission.reviewNotes);
  const anyReviewField = !blank(submission.decision) || !blank(submission.reviewer) || !blank(submission.reviewedAt) || sourceRevisions.length > 0 || !blank(submission.reviewNotes);
...
function submissionAssessment(submission = {}, queueRecord, policy = {}) {
  ...
  const candidateIndex = queueRecord?.variantBindingReviewEvidence?.reviewCandidateVariantIndex;
  const bindingChecks = {
    ...
    sourceDispositionMatches: queueRecord?.sourceDispositionKey === submission.sourceDispositionKey,
    evidencePacketContentHashMatches: queueRecord?.sourceEvidencePacketContentHash === submission.sourceEvidencePacketContentHash,
    evidenceFingerprintMatches: queueRecord?.evidenceFingerprint === submission.evidenceFingerprint,
    reviewCandidateVariantIndexMatches: candidateIndex === submission.reviewCandidateVariantIndex,
    allowedDecisionsMatch: same(submission.allowedDecisions || [], policy.allowedDecisions || []),
    templateStateMatches: submission.state === 'blank_source_bound_variant_binding_review_decision'
  };
  const evidenceTimestamps = [queueRecord?.subjectEvidence?.sourcePageIdentity?.sourceTimestamp, queueRecord?.parentOccurrenceEvidence?.sourcePageIdentity?.sourceTimestamp]
    .filter(value => validIsoTimestamp(value)).map(value => Date.parse(value));
...
function decisionRecord(submission, queueRecord, queueSnapshotContentHash, policy, contentHash = hash) {
  ...
  const base = {
    ...
    decisionKey: `${queueRecord.queueEntryKey}|review-decision|${contentHash(decisionFacts)}`,
    ...
    reviewCandidateVariantIndex: queueRecord.variantBindingReviewEvidence.reviewCandidateVariantIndex,
    ...decisionFacts,
...
function preflight(queueRecords, submissions, policy, queueSnapshotContentHash, contentHash = hash) {
  ...
  const duplicateQueueKeys = duplicates(queueRecords.map(item => item.queueEntryKey));
  ...
  if (accountStateFindings(submissions).length) structuralFailures.push('current_account_state_present');
  ...
  return {
    queueIntegrityFailures, queueOrderValid, queueSnapshotHashValid, duplicateQueueKeys, duplicateSubmissionKeys,
    assessments, completed, blanks, invalid, structuralFailures, atomicAcceptable,
...
export function auditMultiVariantBindingReviewDecisionImport(records = [], { queueRecords = [], submissions = [], policy = {}, queueSnapshotContentHash = '', contentHash = hash } = {}) {
  ...
  if (unsupportedPromotions.length) structuralBlockers.push('decision_import_created_unsupported_variant_binding_semantic_or_optimizer_promotion');
  ...
  return {
    ...
    bindingCoverage: {
      ...
      exactReviewCandidateVariantIndexCount: flight.completed.filter(item => item.assessment.bindingChecks.reviewCandidateVariantIndexMatches).length,
    ...
    semanticPreservationCoverage: {
      ...
      bindingDecisionAppliedCount: records.filter(record => record.bindingReviewDecision !== null || record.boundVariantIndex !== null).length,
    ...
    blockers: unique([
      ...structuralBlockers,
      'recorded_variant_binding_review_decisions_require_separate_semantic_disposition',
      'candidate_member_identity_and_variant_binding_reviews_not_completed',
      'one_or_more_variant_bindings_require_additional_disambiguating_evidence',
...
export function buildMultiVariantBindingReviewDecisionImport({ queueRecords = [], submissions = [], policy = {}, queueSnapshotContentHash = '', contentHash = hash } = {}) {
  const compiled = compileMultiVariantBindingReviewDecisionImportPolicy(policy);
  ...
  return { records, audit: auditMultiVariantBindingReviewDecisionImport(records, { queueRecords, submissions, policy, queueSnapshotContentHash, contentHash }) };
}
