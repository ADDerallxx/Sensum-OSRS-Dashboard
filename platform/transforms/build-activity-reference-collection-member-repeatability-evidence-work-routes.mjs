...
import { buildActivityReferenceCollectionMemberRepeatabilityEvidenceWorkRoutes } from './activity-reference-collection-member-repeatability-evidence-work-routing-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-reference-collection-member-repeatability-evidence-work-routing-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));

async function latestRepeatabilityDispositionSnapshot() {
  const domain = 'activity-reference-collection-member-repeatability-disposition';
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.repeatabilityDispositionAttemptCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('repeatability_disposition_input_not_publishable');
      if (rows.some(record => record.contract !== policy.inputContract)) reasons.push('unexpected_repeatability_disposition_contract');
      if (!rows.length) reasons.push('no_repeatability_disposition_records');
      if (reasons.length) {
  ...
  throw new Error('No valid collection-activity repeatability-disposition snapshot exists.');
}

const input = await latestRepeatabilityDispositionSnapshot();
const built = buildActivityReferenceCollectionMemberRepeatabilityEvidenceWorkRoutes({ dispositionRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_repeatability_disposition_evidence_work_routing_without_identity_or_downstream_promotion',
...
const snapshot = await writeSnapshot(
  root,
  'activity-reference-collection-member-repeatability-evidence-work-routing',
...
const output = path.join(root, 'activity-reference-collection-member-repeatability-evidence-work-routing-audits', generatedAt.replace(/[:.]/g, '-'));
