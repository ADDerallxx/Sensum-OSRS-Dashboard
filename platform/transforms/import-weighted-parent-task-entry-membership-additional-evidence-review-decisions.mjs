...
import { buildWeightedMembershipAdditionalEvidenceReviewDecisionImport } from './weighted-parent-task-entry-membership-additional-evidence-review-decision-import-lib.mjs';
...
const policyFile = 'platform/policies/weighted-parent-task-entry-membership-additional-evidence-review-decision-import-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'weighted-parent-task-entry-membership-additional-evidence-review-queue';
const outputDomain = 'weighted-parent-task-entry-membership-additional-evidence-review-decisions';
...
async function latestQueueSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (queue.reviewQueueEntryCount !== rows.length || queue.blankDecisionTemplateCount !== rows.length
        || queue.missingOutputKeys?.length || queue.unexpectedOutputKeys?.length || queue.duplicateOutputKeys?.length
        || queue.recordMismatches?.length || queue.templateMismatches?.length) {
  ...
  throw new Error('No valid weighted-membership additional-evidence review queue snapshot exists.');
}
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'explicit_fingerprint_bound_parent_section_membership_review_decision_recording_without_semantic_promotion',
