...
import { buildWeightedMembershipAdditionalEvidenceReviewQueueExport } from './weighted-parent-task-entry-membership-additional-evidence-review-queue-export-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/weighted-parent-task-entry-membership-additional-evidence-review-queue-export-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const dispositionDomain = 'weighted-parent-task-entry-membership-additional-evidence-sufficiency-disposition';
const evidenceDomain = 'weighted-parent-task-entry-membership-additional-evidence-source-discovery';
const outputDomain = 'weighted-parent-task-entry-membership-additional-evidence-review-queue';
...
async function latestDispositionSnapshot() {
  ...
  for (const directory of await timestampDirectories()) {
    try {
      ...
      if (audit.dispositionCoverageComplete !== true || audit.weightedMembershipReviewComplete !== false
        ...
        reasons.push('disposition_snapshot_not_publishable_or_gates_not_closed');
      }
      if (!snapshot.rows.length || snapshot.rows.some(row => row.contract !== policy.inputContract || row.state !== policy.inputState
        || row.classification !== policy.eligibleClassification || row.reviewRoute !== policy.eligibleReviewRoute)) {
        reasons.push('unexpected_empty_or_noneligible_input_contract_state_or_route');
      }
  ...
  throw new Error('No valid weighted-membership additional-evidence sufficiency-disposition snapshot exists.');
}
...
async function evidenceSnapshotByHash(expectedHash) {
  ...
  for (const directory of await timestampDirectories()) {
    try {
      ...
      if (audit.evidencePacketAttemptCoverageComplete !== true || audit.candidateSpecificMembershipEvidenceComplete !== true
        || audit.weightedMembershipReviewComplete !== false || audit.optimizerEligibleCount !== 0
        || audit.completeActivityUniverse !== false || audit.publishable !== true) {
        reasons.push('evidence_snapshot_not_publishable_or_gates_not_closed');
      }
      if (!snapshot.rows.length || snapshot.rows.some(row => row.contract !== policy.evidencePacketContract || row.state !== policy.evidencePacketState)) {
  ...
  throw new Error(`No valid additional-evidence source-discovery snapshot matches ${expectedHash}.`);
}
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'deterministic_source_bound_parent_section_membership_review_queue_export_without_review_decision_or_semantic_promotion',
