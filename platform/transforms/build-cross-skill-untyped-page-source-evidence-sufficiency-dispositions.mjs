...
import { buildCrossSkillUntypedPageSourceEvidenceSufficiencyDispositions } from './cross-skill-untyped-page-source-evidence-sufficiency-disposition-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/cross-skill-untyped-page-source-evidence-sufficiency-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'cross-skill-untyped-page-source-evidence';
const outputDomain = 'cross-skill-untyped-page-source-evidence-sufficiency-disposition';
...
async function latestInputSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (audit.sourceEvidenceCoverageComplete !== true || audit.pageTypeReviewComplete !== false ||
        ...
        reasons.push('source_evidence_snapshot_not_publishable_or_semantic_gates_not_closed');
      }
      if (!rows.length || rows.some(row => row.contract !== policy.inputContract || row.state !== 'review_ready')) {
  ...
  throw new Error('No valid cross-skill untyped-page source-evidence snapshot exists.');
}

const input = await latestInputSnapshot();
const built = buildCrossSkillUntypedPageSourceEvidenceSufficiencyDispositions({
...
const source = {
  kind: 'generic_fail_closed_untyped_page_evidence_sufficiency_routing_without_semantic_classification',
