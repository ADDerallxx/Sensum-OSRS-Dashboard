...
import { buildActivityReferenceCollectionMemberRepeatabilityGapDispositions } from './activity-reference-collection-member-repeatability-gap-disposition-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-reference-collection-member-repeatability-gap-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));

async function latestRepeatabilityGapEvidenceSnapshot() {
  const domain = 'activity-reference-collection-member-repeatability-gap-evidence';
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.evidencePacketAttemptCoverageComplete !== true
        || manifest.source?.audit?.repeatabilityGapEvidencePacketCoverageComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('repeatability_gap_evidence_input_not_publishable');
      if (rows.some(record => record.contract !== policy.inputContract)) reasons.push('unexpected_repeatability_gap_evidence_contract');
      if (!rows.length) reasons.push('no_repeatability_gap_evidence_records');
      if (reasons.length) {
  ...
  throw new Error('No valid repeatability-gap evidence snapshot exists.');
}

const input = await latestRepeatabilityGapEvidenceSnapshot();
const built = buildActivityReferenceCollectionMemberRepeatabilityGapDispositions({ gapEvidenceRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_corroborating_repeatability_gap_disposition_without_unscoped_classification_or_downstream_promotion',
...
const snapshot = await writeSnapshot(
  root,
  'activity-reference-collection-member-repeatability-gap-disposition',
...
const output = path.join(root, 'activity-reference-collection-member-repeatability-gap-disposition-audits', generatedAt.replace(/[:.]/g, '-'));
...
console.log(JSON.stringify({
  ...
  coverage: {
    ...
    repeatabilityGapDispositionAttemptCoverageComplete: report.repeatabilityGapDispositionAttemptCoverageComplete,
