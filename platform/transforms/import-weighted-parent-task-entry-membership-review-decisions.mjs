...
import { buildWeightedParentTaskEntryMembershipReviewDecisionImport } from './weighted-parent-task-entry-membership-review-decision-import-lib.mjs';
...
const policyFile = 'platform/policies/weighted-parent-task-entry-membership-review-decision-import-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'weighted-parent-task-entry-membership-review-queue';
const outputDomain = 'weighted-parent-task-entry-membership-review-decisions';
...
async function latestQueueSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (audit.queueExportComplete !== true || audit.weightedMembershipReviewComplete !== false
        || audit.optimizerEligibleCount !== 0 || audit.completeActivityUniverse !== false || audit.publishable !== true) {
        reasons.push('review_queue_snapshot_not_publishable_or_gates_not_closed');
      }
      const routeTotal = Number(partition.reviewQueueEntryCount) + Number(partition.variantScopeEvidenceQueueEntryCount)
        + Number(partition.additionalMembershipEvidenceQueueEntryCount);
      if (partition.reviewQueueEntryCount !== rows.length || partition.blankDecisionTemplateCount !== rows.length
        || partition.totalQueueEntryCount !== routeTotal || partition.missingDispositionKeys?.length
        || partition.unexpectedDispositionKeys?.length || partition.duplicateQueueDispositionKeys?.length
        || partition.crossQueueKeys?.length) {
        reasons.push('weighted_membership_queue_partition_not_exact_or_expected');
      }
  ...
  throw new Error('No valid weighted parent-task entry membership review queue snapshot exists.');
}
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'explicit_fingerprint_bound_weighted_membership_review_decision_recording_without_semantic_promotion',
