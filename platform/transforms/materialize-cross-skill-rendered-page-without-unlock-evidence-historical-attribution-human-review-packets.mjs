...
import { buildHistoricalAttributionHumanReviewPackets } from './cross-skill-rendered-page-without-unlock-evidence-historical-attribution-human-review-packet-materialization-lib.mjs';

const argument = name => process.argv.find(value => value.startsWith(`--${name}=`))?.slice(name.length + 3);
const root = path.resolve(argument('root') || '.platform-data');
const policyFile = 'platform/policies/cross-skill-rendered-page-without-unlock-evidence-historical-attribution-human-review-packet-materialization-v1.json';
...
const built = buildHistoricalAttributionHumanReviewPackets({
...
if (!built.audit.publishable) {
  ...
  const source = {
    kind: 'deterministic_human_readable_evidence_keyed_historical_attribution_review_packet_materialization_without_decision_or_downstream_promotion',
  ...
  console.log(JSON.stringify({
    ...
    coverage: {
      ...
      historicalRenderedObservationCount: report.evidenceCoverage.historicalRenderedObservationCount,
      historicalGuideRevisionBindingCount: report.evidenceCoverage.historicalGuideRevisionBindingCount,
