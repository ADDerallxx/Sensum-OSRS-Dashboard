...
import { buildActivityCanonicalSubjectScopeEvidenceWorkRoutes } from './activity-canonical-subject-scope-evidence-work-routing-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/activity-canonical-subject-scope-evidence-work-routing-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const domain = 'activity-infobox-schema-semantics-disposition';
...
async function latestDispositionSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.canonicalActivitySubjectBindingReviewComplete !== true
        || manifest.source?.audit?.publishable !== true) reasons.push('subject_binding_disposition_not_publishable');
      if (rows.some(record => record.contract !== policy.inputContract)) reasons.push('unexpected_subject_binding_disposition_contract');
      if (rows.some(record => record.state !== policy.eligibleInputState)) reasons.push('one_or_more_records_lack_source_supported_subject_binding');
      if (!rows.length) reasons.push('no_subject_binding_disposition_records');
  ...
  throw new Error('No valid source-supported canonical-activity subject-binding disposition snapshot exists.');
}

const input = await latestDispositionSnapshot();
const built = buildActivityCanonicalSubjectScopeEvidenceWorkRoutes({ dispositionRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_bound_canonical_activity_subject_scope_evidence_work_routing_without_scope_repeatability_or_downstream_promotion',
...
const snapshot = await writeSnapshot(
  root,
  'activity-canonical-subject-scope-evidence-work-routing',
...
const output = path.join(root, 'activity-canonical-subject-scope-evidence-work-routing-audits', generatedAt.replace(/[:.]/g, '-'));
