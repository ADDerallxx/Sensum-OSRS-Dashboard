const rules=[
  ...
  {axis:'equipment_modifier',objective:'multi_objective',test:text=>/\b(?:gloves?|boots?|capes?|rings?|focus|grapple|symbols?|hammers?|suits?|crossbows?|light sources?)\b/i.test(text)&&/(?:experience|xp|rate|success|time|failure|penalty|marks?|require|required|access)/i.test(text)},
...
export function auditVariantAxes({records,members,variants}){
  const roles=new Map(members.map(x=>[x.name,x.entity_role])),variantParents=new Map();
  for(const row of variants){if(!variantParents.has(row.parent_name))variantParents.set(row.parent_name,new Set());variantParents.get(row.parent_name).add(row.variant_key)}
  const findings=[];
  for(const record of records){if(roles.get(record.name)==='reference_collection')continue;const fragments=record.evidence_fragments||[];
    for(const rule of rules){const matched=fragments.filter(x=>rule.test(x.text||'')).slice(0,8);if(!matched.length)continue;const expanded=rule.axis==='course_branch'&&variantParents.get(record.name)?.has('basic')&&variantParents.get(record.name)?.has('advanced');findings.push({name:record.name,axis:rule.axis,objective:rule.objective,state:expanded?'expanded':'requires_model_variant',source_revision:record.source_revision,source_url:record.source_url,source_locators:matched.map(x=>({line:x.line,excerpt:x.text}))})}
  }
