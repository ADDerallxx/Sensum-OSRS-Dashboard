...
const report={contract:'sensum.activity-family-build.v1',generatedAt:new Date().toISOString(),input:input.dir,classifierVersion:'family-rules-v5',parserVersion:'family-regex-v6',variantParentsDiscovered:variantParents.size,variantSnapshotRejections:variantState.rejections,records:input.rows.length,memberships:members.length,classifications:{classifiedRecords:classifiedKeys.size,unclassifiedRecords:unclassifiedKeys.size,referenceCollections:members.filter(x=>x.entity_role==='reference_collection').length,compositeMethods:members.filter(x=>x.entity_role==='composite_method').length},candidateFacts:facts.length,verifiedFacts:0,absoluteBestActivityGate:'blocked',families,contentHash:hash({members,facts})};
