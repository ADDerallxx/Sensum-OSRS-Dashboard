...
import { buildIndependentScopedActivityRepeatabilityEvidenceDispositions } from './independent-scoped-activity-repeatability-evidence-disposition-lib.mjs';

const root = path.resolve(process.argv.find(argument => argument.startsWith('--root='))?.slice(7) || '.platform-data');
const policyFile = 'platform/policies/independent-scoped-activity-repeatability-evidence-disposition-v1.json';
const policy = JSON.parse(await fs.readFile(path.resolve(policyFile), 'utf8'));
const inputDomain = 'independent-scoped-activity-repeatability-evidence';
const outputDomain = 'independent-scoped-activity-repeatability-evidence-disposition';
...
async function latestEvidenceSnapshot() {
  ...
  for (const directory of directories) {
    try {
      ...
      if (manifest.source?.audit?.evidencePacketAttemptCoverageComplete !== true
        || manifest.source?.audit?.independentEvidenceDiscoveryScopeComplete !== true
        || manifest.source?.audit?.repeatabilityReviewComplete !== false
        || manifest.source?.audit?.publishable !== true) reasons.push('independent_repeatability_evidence_not_complete_or_publishable');
  ...
  throw new Error('No valid independent scoped-activity repeatability-evidence snapshot exists.');
}

const input = await latestEvidenceSnapshot();
const built = buildIndependentScopedActivityRepeatabilityEvidenceDispositions({ evidenceRecords: input.rows, policy });
const inputSnapshot = { directory: input.directory, contentHash: input.manifest.contentHash, rejections: input.rejections };
const source = {
  kind: 'generic_independent_scoped_activity_repeatability_signal_and_domain_disposition_without_cross_line_join_or_verdict',
