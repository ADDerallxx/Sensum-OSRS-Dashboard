...
import { buildAgilityTargetConditionGapSourceChannelChangeEvaluation } from './agility-target-condition-gap-source-channel-change-evaluation-lib.mjs';
...
const policyFile = path.resolve('platform/policies/agility-target-condition-gap-source-channel-change-evaluation-v1.json');
const registryPolicyFile = path.resolve('platform/policies/agility-target-condition-gap-source-channel-change-monitoring-registry-v1.json');
...
const built = buildAgilityTargetConditionGapSourceChannelChangeEvaluation({ baseline, current, policy, registryPolicy, contentHash: hash });
...
if (built.audit.publishable) {
  const snapshot = await writeSnapshot(root, policy.outputDomain, built.records, {
    kind: 'deterministic_fail_closed_baseline_vs_refresh_source_channel_change_evaluation',
    scope: 'change events for exact Agility target-condition blocker/channel pairs only; empty snapshot is a valid unchanged result, never a game-fact assertion',
