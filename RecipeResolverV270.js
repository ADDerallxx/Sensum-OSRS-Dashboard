const V270_RECIPE_USER_AGENT='SensumOSRSDashboard/2.70 (Wiki-backed recipe resolver)';
...
function v270RecipeFromHtml_(html,selected,meta){
  ...
  tables.forEach(table=>{
    ...
    for(let i=headerIndex+1;i<rows.length;i++){
      ...
      const itemCell=row.find(c=>/<a\b/i.test(c.html))||row[0],item=v270ItemName_(itemCell.html),quantityCell=row.find((c,index)=>index>0&&/\d/.test(c.text)),quantity=Math.max(0,v270Number_(quantityCell&&quantityCell.text));if(!item||!quantity)continue;
...
function v270ResolveWikiRecipe_(item){
  const selected={id:Number(item&&item.id||0),name:String(item&&item.name||item||'').trim()};if(!selected.name)return null;const cache=CacheService.getScriptCache(),key='V270_RECIPE_'+Utilities.base64EncodeWebSafe(selected.name.toLowerCase()).slice(0,150),cached=cache.get(key);if(cached)return cached==='null'?null:JSON.parse(cached);
  try{
