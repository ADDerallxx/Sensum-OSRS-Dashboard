...
check(/V3\.07 · Modular Money/.test(html) && /Finish line:/.test(html) && /Average path readiness/.test(html), 'Audited goal cards must expose finish lines and path-readiness summaries.');
check(/function getV307MoneyState\(\)/.test(dashboardSource) && /money:\{status:'idle'\}/.test(html) && /\.getV307MoneyState\(\)/.test(html), 'Money journals must load through the V3.07 lazy module endpoint.');
