...
function getV240Portfolio(){
  ...
  purchases.forEach(p=>{const k=key(p),row=positions[k]||(positions[k]={itemId:p.itemId,item:p.item,bought:0,purchaseCost:0,disposed:0,purposeMap:{}});row.bought+=p.quantity;row.purchaseCost+=p.quantity*p.unitPrice;row.purposeMap[p.purpose||'Other']=true;});
  actions.forEach(a=>{const k=key(a),row=positions[k]||(positions[k]={itemId:a.itemId,item:a.item,bought:0,purchaseCost:0,disposed:0,purposeMap:{}});row.disposed+=a.quantity;});
  const list=Object.keys(positions).map(k=>{const x=positions[k],averageCost=x.bought?x.purchaseCost/x.bought:0;return Object.assign(x,{averageCost:averageCost,remaining:Math.max(0,x.bought-x.disposed),purposes:Object.keys(x.purposeMap||{})})});
  const costByKey={};list.forEach(x=>costByKey[key(x)]=x.averageCost);
  const enriched=actions.map(a=>{const avg=Number(a.costBasis||costByKey[key(a)]||0),extra=/^alch$/i.test(a.action)?a.natureCost:a.unitTax,profit=(a.unitReturn-avg-extra)*a.quantity;return Object.assign({},a,{averageCost:avg,totalReturn:a.unitReturn*a.quantity,totalCost:avg*a.quantity,realizedProfit:profit,magicXp:/^alch$/i.test(a.action)?a.quantity*65:0})}).sort((a,b)=>b.date.localeCompare(a.date)||b.updatedAt.localeCompare(a.updatedAt));
