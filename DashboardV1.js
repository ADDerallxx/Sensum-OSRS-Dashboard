...
function getV243ProcessingBatches(){
  ...
  return sh.getRange(2,1,sh.getLastRow()-1,16).getValues().map(r=>{let inputs=[];try{inputs=JSON.parse(String(r[3]||'[]'))}catch(e){}const planned=Number(r[6]||0),processed=Number(r[7]||0),sold=Number(r[8]||0),waste=Number(r[14]||0),unitCost=inputs.reduce((s,x)=>s+Number(x.quantity||0)*Number(x.unitCost||0),0),tax=Math.min(5000000,Math.floor(Number(r[9]||0)*.02)),ready=Math.max(0,processed-sold-waste),realized=sold*(Number(r[9]||0)-tax-unitCost),potential=ready*(Number(r[9]||0)-tax-unitCost),status=sold+waste>=processed&&processed>=planned?'Completed':ready>0&&sold>0?'Partially sold':ready>0?'Ready to sell':processed>0?'Partially processed':'Planned';return {id:String(r[0]||''),date:r[1] instanceof Date?Utilities.formatDate(r[1],Session.getScriptTimeZone()||'America/Denver','yyyy-MM-dd'):String(r[1]||''),recipe:String(r[2]||''),inputs:inputs,outputItemId:Number(r[4]||0),outputItem:String(r[5]||''),planned:planned,processed:processed,sold:sold,sellPrice:Number(r[9]||0),xpEach:Number(r[10]||0),notes:String(r[11]||''),updatedAt:r[13] instanceof Date?r[13].toISOString():String(r[13]||''),waste:waste,status:status,unitCost:unitCost,ready:ready,inputSetsRemaining:Math.max(0,planned-processed),realizedProfit:realized,potentialProfit:potential,xp:processed*Number(r[10]||0)}}).filter(x=>x.id&&x.outputItem).sort((a,b)=>b.date.localeCompare(a.date)||b.updatedAt.localeCompare(a.updatedAt));
}
function saveV243ProcessingBatch(entry){
  entry=entry||{};const date=String(entry.date||''),recipe=String(entry.recipe||'').trim(),output=String(entry.outputItem||'').trim(),planned=Math.floor(Number(entry.planned)||0),processed=Math.floor(Number(entry.processed)||0),sold=Math.floor(Number(entry.sold)||0),waste=Math.floor(Number(entry.waste)||0),sellPrice=Math.floor(Number(entry.sellPrice)||0),xpEach=Number(entry.xpEach)||0,inputs=(entry.inputs||[]).map(x=>({item:String(x.item||'').trim(),quantity:Number(x.quantity)||0,unitCost:Math.max(0,Number(x.unitCost)||0)})).filter(x=>x.item&&x.quantity>0);if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error('Choose a valid batch date.');if(!recipe||!output)throw new Error('Enter a recipe and output item.');if(!inputs.length)throw new Error('Add at least one valid input.');if(planned<1||processed<0||processed>planned)throw new Error('Processed quantity must be between zero and the planned quantity.');if(sold<0||waste<0||sold+waste>processed)throw new Error('Sold plus waste cannot exceed processed output.');
  const lock=LockService.getScriptLock();lock.waitLock(10000);try{const sh=v243ProcessingSheet_(),id=String(entry.id||Utilities.getUuid()),row=[id,new Date(date+'T12:00:00'),recipe,JSON.stringify(inputs),Number(entry.outputItemId||0),output,planned,processed,sold,sellPrice,xpEach,String(entry.notes||''),String(entry.source||'Dashboard processing journal'),new Date(),waste,String(entry.status||'')],ids=sh.getLastRow()>1?sh.getRange(2,1,sh.getLastRow()-1,1).getDisplayValues():[],index=ids.findIndex(r=>String(r[0])===id);if(index>=0)sh.getRange(index+2,1,1,16).setValues([row]);else sh.appendRow(row);return {ok:true,id:id,batches:getV243ProcessingBatches()};}finally{lock.releaseLock();}
}
