...
function saveV240Purchase(entry){
  ...
  const lock=LockService.getScriptLock();lock.waitLock(10000);try{const sh=v240PurchaseSheet_(),id=String(entry.id||Utilities.getUuid()),row=[id,new Date(date+'T12:00:00'),Number(entry.itemId||0),item,quantity,unitPrice,String(entry.purpose||'Other'),String(entry.source||'Dashboard'),new Date()],values=sh.getLastRow()>1?sh.getRange(2,1,sh.getLastRow()-1,1).getDisplayValues():[],index=values.findIndex(r=>String(r[0])===id);if(index>=0)sh.getRange(index+2,1,1,9).setValues([row]);else sh.appendRow(row);return {ok:true,id:id,purchases:getV240Purchases()};}finally{lock.releaseLock();}
}
function deleteV240Purchase(id){
  id=String(id||'');if(!id)throw new Error('Choose a purchase.');const lock=LockService.getScriptLock();lock.waitLock(10000);try{const sh=v240PurchaseSheet_();if(sh.getLastRow()>1){const ids=sh.getRange(2,1,sh.getLastRow()-1,1).getDisplayValues(),index=ids.findIndex(r=>String(r[0])===id);if(index>=0)sh.deleteRow(index+2);}return {ok:true,purchases:getV240Purchases()};}finally{lock.releaseLock();}
}
