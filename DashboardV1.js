...
function getV243ProcessingBatches(){
  ...
  return sh.getRange(2,1,sh.getLastRow()-1,17).getValues().map(r=>{let inputs=[],modifiers=[];try{inputs=JSON.parse(String(r[3]||'[]'))}catch(e){}try{modifiers=JSON.parse(String(r[16]||'[]'))}catch(e){}const supplied=new Set(modifiers.flatMap(m=>m.supplies||[]).map(x=>String(x).toLowerCase())),effectiveInputs=inputs.map(x=>Object.assign({},x,{effectiveQuantity:supplied.has(String(x.item).toLowerCase())?0:Number(x.quantity||0)})),planned=Number(r[6]||0),processed=Number(r[7]||0),sold=Number(r[8]||0),waste=Number(r[14]||0),unitCost=effectiveInputs.reduce((s,x)=>s+Number(x.effectiveQuantity||0)*Number(x.unitCost||0),0),tax=Math.min(5000000,Math.floor(Number(r[9]||0)*.02)),ready=Math.max(0,processed-sold-waste),realized=sold*(Number(r[9]||0)-tax-unitCost),potential=ready*(Number(r[9]||0)-tax-unitCost),status=sold+waste>=processed&&processed>=planned?'Completed':ready>0&&sold>0?'Partially sold':ready>0?'Ready to sell':processed>0?'Partially processed':'Planned';return {id:String(r[0]||''),date:r[1] instanceof Date?Utilities.formatDate(r[1],Session.getScriptTimeZone()||'America/Denver','yyyy-MM-dd'):String(r[1]||''),recipe:String(r[2]||''),inputs:inputs,effectiveInputs:effectiveInputs,modifiers:modifiers,outputItemId:Number(r[4]||0),outputItem:String(r[5]||''),planned:planned,processed:processed,sold:sold,sellPrice:Number(r[9]||0),xpEach:Number(r[10]||0),notes:String(r[11]||''),updatedAt:r[13] instanceof Date?r[13].toISOString():String(r[13]||''),waste:waste,status:status,unitCost:unitCost,ready:ready,inputSetsRemaining:Math.max(0,planned-processed),realizedProfit:realized,potentialProfit:potential,xp:processed*Number(r[10]||0)}}).filter(x=>x.id&&x.outputItem).sort((a,b)=>b.date.localeCompare(a.date)||b.updatedAt.localeCompare(a.updatedAt));
}
